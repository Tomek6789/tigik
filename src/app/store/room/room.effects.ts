...
import { tap, switchMap, map, mergeMap, filter, catchError } from "rxjs/operators";
import { from, of } from "rxjs";
import { createRoom, finishGame, getRoom, getRoomFailure, getRoomSuccess, joinRoom, joinRooomSuccess, playerLeaveRoom, selectedElement, startGame, startGameSuccess } from "./room.actions";
import { bestScoreSelector, isUserLoginSelector, roomUidSelector, scoreSelector, userUidSelector } from "../user/user.selectors";
...
import {  roomPlayersSelector, searchingElementSelector } from "./room.selectors";
...
export class RoomEffects {
    ...
    getRoom$ = createEffect(() => this.actions$.pipe(
        ofType(getRoom),
        switchMap(({ roomUid }) => {
            return this.callableFunctions.checkRoomExists()({ roomUid }).
                pipe(
                    switchMap(() => {
                    ...
                    catchError(() => {
                        this.snackBarService.openSnackBar('Room not exists, please get new invitation')
    ...
    playerLeaveRoom$ = createEffect(() => this.actions$.pipe(
        ...
        concatLatestFrom(() => [
            this.store.select(userUidSelector),
            this.store.select(roomUidSelector),
            this.store.select(roomPlayersSelector).pipe(filter<string[]>(Boolean)),
        ]),
